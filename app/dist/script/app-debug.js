define("dist/script/app-debug", [ "platformer-debug", "./entrance-debug", "./data-debug" ], function(require, exports, module) {
    var Platformer = require("platformer-debug");
    var entrance = require("./entrance-debug");
});

define("dist/script/entrance-debug", [ "platformer-debug", "dist/script/data-debug" ], function(require, exports, module) {
    var Platformer = require("platformer-debug");
    var data = require("dist/script/data-debug");
    ...
    var man = new Platformer.Sprite("Man", manArtist, [ manRunBehavior, manJumpBehavior, manFallBehavior, manCollideBehavior, manMoveBehavior ]);
    man.hide();
    ...
    platform.add([ skyCanvas, floorCanvas, backgroundCanvas, mainCanvas, cloudCanvas, entityCanvas, sky, grove, floor, banner, man, cloudOne, cloudTwo, cloudThree, cloudFour, cloudFive, cloudSix, cloudSeven, assistant, contactMan, qrcodeMM, printer, desk, flag, flowerOne, flowerTwo, barrier, coinOne, coinTwo, coinThree, coinFour, coinFive, coinSix ]).draw();
    ...
    platform.update(function() {
        ...
        if (pressedKeys[37]) {
            // Left
            man.runToLeft(10);
            backgroundCanvas.moveToLeft(200);
            // pix / s
            entityCanvas.moveToLeft(200);
            // pix / s
            cloudCanvas.moveToLeft(150);
        } else if (pressedKeys[39]) {
            // Right
            man.runToRight(10);
            backgroundCanvas.moveToRight(200);
            // pix / s
            entityCanvas.moveToRight(200);
            // pix / s
            cloudCanvas.moveToRight(150);
        }
        // 上下同时按下时，只执行上
        if (pressedKeys[38]) {
            // Up
...
define("dist/script/data-debug", [], function(require, exports) {
    exports.grove = [ {
        left: 0,
        top: 0,
        width: 5782,
